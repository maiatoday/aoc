...
object Day11 : Day<BigInteger, String> {
    ...
    data class Monkey(
        ...
        fun round(doWorryDrop: Boolean = false): List<MonkeySwap> {
//            val itemValues = items.map { operation(it) }.map { if (doWorryDrop) worryDrop(it) else it }
            val itemValues = if (doWorryDrop) items.map { operation(it) }.map { worryDrop(it)} else items.map { operation(it) }
    ...
    private fun String.toOperation(): (BigInteger) -> BigInteger {
        ...
        return when {
            (this == "old * old") -> { x: BigInteger -> x * x }
            (this.startsWith("old + ")) -> { x: BigInteger -> x + term2 }
            (this.startsWith("old * ")) -> { x: BigInteger -> x * term2 }
            else -> {
                error("Oops!")
            }
...
//            swaps.inspect(index)
    ...
    override fun part2(input: String): BigInteger {
        val roundCheck = listOf(0, 1, 3, 20, 700,999,1999, 2999)// 19, 20, 21, 1999, 2999, 3999)
